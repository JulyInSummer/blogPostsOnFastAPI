...
from app.schemas import Post, PostsOut, PostOut, PostOutVotes
...
@router.get("/", response_model=List[PostOutVotes])
...
@router.get("/{id}", response_model=PostOutVotes)
...
def delete_post(id: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if post.owner_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You are not authorized to perform requsted action")
